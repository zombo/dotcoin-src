    ...
    model(0)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::dotcoinAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::dotcoinAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CDotcoinAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CDotcoinAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CDotcoinAddress(key.GetPubKey().GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
