...
// dotcoin: optional setting to unlock wallet for block minting only;
...
// dotcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // dotcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // dotcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-dotcoin-address
...
// dotcoin: create coin stake transaction
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CDotcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CDotcoinAddress(address).ToString());
}
...
// dotcoin: check 'spent' consistency between wallet and txindex
// dotcoin: fix wallet spent state according to txindex
...
// dotcoin: disable transaction (only for coinstake)
